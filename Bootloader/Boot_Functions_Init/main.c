...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan1)
{
	...
	Header_ID[RxCounter] = RxHeader.StdId;

	for(uint8_t i = 0 ; i < 8 ; i++)
	{
	g_dataBuffer[(RxCounter*8)%1000+i] = RxData[i];
	}
...
int main(void)
{
  ...
  CommandCounter = 150 <= RxCounter = 200+1000;
  buffer_full = 0 && CommandCounter >= RxCounter;
  if(((buffer_full == 0) && (CommandCounter <= RxCounter)) || ((buffer_full == 1) && (CommandCounter >= RxCounter)))
  {
...
static void bootloader_handle_go_cmd(void)
{
	...
	uint32_t*ptr_address = &g_dataBuffer[CommandCounter*8];
...
static void bootloader_handle_flash_erase_cmd(void)
{
    ...
    uint32_t*ptr = &g_dataBuffer[CommandCounter*8];
...
{
	uint32_t*ptr_size = &g_dataBuffer[CommandCounter*8];
...
static void bootloader_handle_mem_write_data_cmd(void)
{
	...
	{
        //execute mem write
		WRITE_STATUS = execute_mem_write(mem_address, payload_len,&g_dataBuffer[CommandCounter*8]);
...
{
	if((buffer_full == 0) && (CommandCounter == RxCounter))
	{
		//nothing
	}
	else
	{
	    switch(Header_ID[CommandCounter])
	    {
	    case BL_GET_HELP:
...
uint8_t execute_mem_write(uint32_t mem_address, uint32_t len , uint32_t*write_buffer)
{
...
static void check_rx_counter(void)
{
	if(RxCounter < 125)
	{
		RxCounter++;
...
static void check_command_counter(void)
{
	if(CommandCounter < 125)
	{
		CommandCounter++;
	}
	else
